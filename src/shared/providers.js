...
function buildSystemPrompt(conversation, options = {}) {
  ...
  const base = [
    ...
    "对数学、代码、概念解释类问题，请拆步骤说明，指出隐含前提，并在有帮助时给一个更简单的例子。",
	    "你必须使用标准的 Markdown 格式组织回答，渲染引擎只接受严格的 CommonMark / GFM 语法：",
	    "- 标题：使用 # 加空格开头（正确：# 标题，错误：#标题）",
	    "- 代码块：三个反引号围栏并标注语言（```javascript），不要省略语言标签",
	    "- 表格：必须包含表头分隔行（| --- | --- |），否则表格无法显示",
	    "- 列表：无序列表用 - 加空格，有序列表用 1. 加空格",
	    "- 分割线：独立一行的 ---，前后各空一行",
	    "- 引用：使用 > 加空格开头",
	    "如果输出代码、配置、命令或示例数据，必须使用正常的多行格式，保留换行和基本缩进；不要把 Java、YAML、JSON、SQL、注解或配置项压缩成单行。",
